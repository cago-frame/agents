...
// 本文件定义 codex runner-level 的 native Option 构造器:Tools /
...
// 到 Codex 的运行时上下文,而不是 app-server 进程级配置(那部分在 options.go)。
//
...
// SubagentStop 注册一个 SubagentStop hook(当前 Codex app-server 不会触发)。
