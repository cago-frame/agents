...
func (c *appClient) Call(ctx context.Context, method appMethod, params any) (json.RawMessage, error) {
	id := c.nextID.Add(1)
...
func (c *appClient) Notify(_ context.Context, method appMethod, params any) error {
...
func (c *appClient) routeMessage(msg rpcMessage) bool {
	...
	in := appInbound{Method: msg.Method, Params: msg.Params}
	if len(msg.ID) > 0 {
		in.Kind = appInboundRequest
		in.ID = append(json.RawMessage(nil), msg.ID...)
	} else {
		in.Kind = appInboundNotification
	}
	select {
	case c.incoming <- in:
		return true
