...
// HookStage 是 claudecode hook 触发的生命周期位点。语义对齐 Claude Code CLI。
...
// HookFunc 是 claudecode hook 回调签名。
type HookFunc func(ctx context.Context, in HookInput) (*HookOutput, error)

// HookInput 是 hook 拿到的运行时上下文。所有字段都是 best effort。
...
// hookSpec 是内部存储:Runner.New 收集所有 hook 注册,在构造 Runtime hook 链时翻译。
...
// wrapHookFunc bridges a claudecode-native HookFunc to a runtime.HookFunc.
func wrapHookFunc(fn HookFunc) runtime.HookFunc {
	return func(ctx context.Context, rin runtime.HookInput) (runtime.HookOutput, error) {
		in := HookInput{
			Stage:        HookStage(stageRuntimeToCC(rin.Stage)),
			Cwd:          rin.Cwd,
			Prompt:       rin.Prompt,
...
// stageRuntimeToCC converts a runtime.HookStage to the claudecode public
// HookStage names ("PreToolUse" / "PostToolUse" / etc.).
func stageRuntimeToCC(s runtime.HookStage) string {
	switch s {
...
// stageCCToRuntime converts the claudecode-public HookStage value to runtime.HookStage.
func stageCCToRuntime(s HookStage) runtime.HookStage {
	switch s {
